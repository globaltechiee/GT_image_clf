...
def get_filenames(class_labels):
    ...
    for c in class_labels:
        ...
        imgs += glob('downloads/*%s*/gt_?????.jpg' % folder_id)
...
def collect_images(class_labels, suffix='photo,close up,portrait'):
    ...
    for label in class_labels:
        ...
        # Collect a list of all files, starting with 'gt_' ones
        imgs = sorted(glob(os.path.join('downloads', 'images_*%s*' % folder_id, '*')))
        imgs_gt = [f for f in imgs if 'gt_' in f]
        imgs_new = [f for f in imgs if 'gt_' not in f]
        imgs = imgs_gt + imgs_new
        ...
        for i, img in enumerate(imgs):
            ...
            # Define 'gt_' file name
            filename = os.path.join(os.path.dirname(img), 'gt_%05d.jpg' % (i + 1))
            ...
            if np.abs(width-height)>5:
                ...
                # Store image file with 'gt_' prefix if it doesn't have it yet
                im.save(filename)
